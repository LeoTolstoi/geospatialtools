    ...
    while flag == False:
        ...
        for var in Xd:
            ...
            tmp = normalize_variable(np.copy(Xd[var]['d']), Xd[var]['min'],
                                     Xd[var]['max'])
            tmp = ws[Xd.keys().index(
                var)] * tmp  #(tmp-np.min(tmp))/(np.max(tmp)-np.min(tmp))
            X.append(tmp)
        X = np.array(X).T
...
def create_basin_tiles(basin_clusters, hand, basins, dh):
    ...
    for ubc in ubcs:
        ...
        #Compute the edges
        pedges = 2.5
        bin_edges = np.linspace(0.0,
                                np.max(data)**(1.0 / float(pedges)),
                                nbins + 1)**pedges
        #compute the binning
...
def compute_polygon_info(polygons, clusters, res):
    ...
    x = np.linspace(res / 2, polygons.shape[0] * res - res / 2,
                    polygons.shape[0])
    y = np.linspace(res / 2, polygons.shape[1] * res - res / 2,
                    polygons.shape[1])
    ...
    #Assemble i/o arrays
    pcxy = np.zeros((np.int(np.max(polygons)) + 1, 2),
    ...
    pmatrix = scipy.sparse.coo_matrix(
        (np.ones(pd2o.shape[0]), (pd2o[:, 0], pd2o[:, 1])),
        shape=(np.int(np.max(polygons)) + 1, np.int(np.max(polygons)) + 1),
        dtype=np.float32)
    pmatrix = pmatrix.tocsr()

    #Compute length between centroids
    tmp = scipy.sparse.find(pmatrix)
    dist = ((pcxy[tmp[0], 0] - pcxy[tmp[1], 0])**2 +
            (pcxy[tmp[0], 1] - pcxy[tmp[1], 1])**2)**0.5
...
def calculate_channel_properties(channels, channel_topology, slope, eares,
                                 mask):

    #Convert to 0-index
    channel_topology[channel_topology >
                     0] = channel_topology[channel_topology > 0] - 1
    ...
    db_channels = {
        ...
        'manning': channel_mannings,
        'width': channel_width,
        'bankfull': channel_bankfull,
