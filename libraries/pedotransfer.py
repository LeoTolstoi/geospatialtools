...
def Residual_Water_Content_Maidment92(phi, clay, sand):
    ...
    # Maidment,1992
    SMR = (-0.0182482 + 0.00087269 * sand + 0.00513488 * clay +
           0.02939286 * phi - 0.00015395 * clay * clay -
           0.0010827 * sand * phi - 0.00018233 * clay * clay * phi * phi +
           0.00030703 * clay * clay * phi - 0.0023584 * phi * phi * clay)
    return SMR
...
def Theta_1500t_Saxton2006(S, C, OM):
    ...
    return (-0.024 * S + 0.487 * C + 0.006 * OM + 0.005 * S * OM -
...
def Theta_33t_Saxton2006(S, C, OM):
    ...
    return (-0.251 * S + 0.195 * C + 0.011 * OM + 0.006 * S * OM -
...
def Theta_S33t_Saxton2006(S, C, OM):
    ...
    return (0.278 * S + 0.034 * C + 0.022 * OM - 0.018 * S * OM -
...
def Run_Tests():
    ...
    sand = np.array([
        5.0, 5.0, 10.0, 20.0, 15.0, 5.0, 25.0, 35.0, 35.0, 45.0, 40.0, 50.0
    ]) / 100
    om = 2.5
    ksat = np.array(
        [108., 96.7, 50.3, 15.5, 16.1, 22., 11.3, 4.3, 5.7, 3.7, 1.4, 1.1])
    thetas = np.array(
        [46., 46., 45., 46., 48., 48., 43., 48., 51., 52., 44., 50.]) / 100
    theta33 = np.array(
        [10., 12., 18., 28., 31., 30., 27., 36., 38., 41., 36., 42.]) / 100
    theta1500 = np.array(
        [5., 5., 8., 14., 11., 6., 17., 22., 22., 27., 25., 30.]) / 100
    ...
    print(np.allclose(theta33, Theta_33_Saxton2006(clay, sand, om),
                      atol=1e-02))
    print(theta33)
    print(Theta_33_Saxton2006(clay, sand, om))
    print("Comparing theta1500")
    print(
        np.allclose(theta1500,
                    Theta_1500_Saxton2006(clay, sand, om),
                    atol=1e-02))
    print(theta1500)
